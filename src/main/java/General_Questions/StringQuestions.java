...
public class StringQuestions {
	...
	// two pointer O(n) time O(1) space solution, one pass converting T to F, another converting F to T, return the max
	public static int maxConsecutiveAnswers(String answerKey, int k) {
		return Math.max(checkConsecutive(answerKey, k, 'T'), checkConsecutive(answerKey, k, 'F'));
	}
	private static int checkConsecutive(String answerKey, int k, char convert) {
		int lo = 0;
		int hi = 0;
		int subs = 0;
		int max = 0;
		while (hi < answerKey.length()) {
			if (answerKey.charAt(hi) == convert) { // convert
				while (subs == k) { // no capacity for conversion
					if (answerKey.charAt(lo++) == convert) {
						subs--; // found capacity for conversion
					}
				}
				subs++;
			}
			hi++;
			max = Math.max(max, hi-lo); // hi is incremented 1 above the known good range so don't need to +1 to range
		}
		return max;
	}

	// worst case runtime O(n) where n is the sum of all chars in arr
	public static String longestCommonSuffix(String[] arr) {
		...
		while (true) {
			...
			for (int i = 0; i < arr.length; i++) {
				int charI = arr[i].length() - j;
				if (charI == -1) {
					return result.reverse().toString();
				}
				if (i==0) {
					c = arr[i].charAt(charI);
				} else {
					if (arr[i].charAt(charI) != c) {
						return result.reverse().toString();
					}
				}
			}
