...
public class Array2DQuestions {
	...
	public static int[][] onesMinusZeros(int[][] grid) {
		int[] onesRow = new int[grid.length]; // onesRow[i] is the number of ones in row i
...
//		System.out.println(Arrays.toString(onesCol));
		...
		for (int y = 0; y < diff.length; y++) {
			for (int x = 0; x < diff[0].length; x++) {
				int zerosRow = grid[0].length - onesRow[y];
				int zerosCol = grid.length - onesCol[x];
				diff[y][x] = onesCol[x] + onesRow[y] - zerosRow - zerosCol;
			}
		}
		return diff;
	}
